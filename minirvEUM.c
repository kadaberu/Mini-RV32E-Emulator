...
void draw(uint32_t color) {
 ...
 int width=256;
  ...
  io_write(AM_GPU_FBDRAW,w,h,&pixels[h*width+w],1,1,true);
}

void display(){
    
    if(store_address>0x20000000&&store_address<0x20040000&&sw==1){
        RGB=GPR[rs2]&0xFFFFFF;
        w=(store_address>>2)&0xFF;
        h=(store_address>>10)&0xFF;
  ...
  if(fp_ram==NULL){
    ...
    fread(RAM,1,4*1048576,fp_ram);
        ...
        while(PC/4<1048576){
            ...
            inst_cycle();
            display();AM_INPUT_KEYBRD_T ev=io_read(AM_INPUT_KEYBRD);
